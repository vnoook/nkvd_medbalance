...
class WindowMain(PyQt5.QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        # переменные
        self.info_extention_open_file = 'Файлы ZIP (*.zip)'  # 'Файлы ZIP (*.zip), CSV (*.csv)'
        ...
        # pushButton_report_to_xls
        self.pushButton_report_to_xls = PyQt5.QtWidgets.QPushButton(self)
        self.pushButton_report_to_xls.setObjectName('pushButton_report_to_xls')
        # self.pushButton_report_to_xls.setEnabled(False)
        self.pushButton_report_to_xls.setText('Создать отчёт "Журнал записей пациентов"')
        self.pushButton_report_to_xls.setGeometry(PyQt5.QtCore.QRect(10, 230, 260, 25))
        self.pushButton_report_to_xls.clicked.connect(self.parse_xlsx)
        self.pushButton_report_to_xls.setToolTip(self.pushButton_report_to_xls.objectName())
...
#
    ...
    def units_activate(self):
        ...
        if ((self.text_empty_path_file not in self.label_path_file.text()) and
            ...
            self.pushButton_report_to_xls.setEnabled(True)
